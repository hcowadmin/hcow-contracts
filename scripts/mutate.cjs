...
const MUTATIONS = [
  ...
  {
    name: "7차 M-7: set-root 가 ClaimWindowTooShort 를 미리 보지 않는다",
    file: SR,
    from: "  if (tree.startTime > latestStart) {",
    to:   "  if (false) {",
    run: JS(OG),
    expect: "시작이 claimDeadline − minClaimWindow 보다 1초라도 늦으면 calldata 를 만들지 않는다 (7차 M-7)",
  },
  {
    name: "7차 M-7: set-root 의 창 경계가 컨트랙트보다 한 칸 좁다",
    file: SR,
    from: "  if (tree.startTime > latestStart) {",
    to:   "  if (tree.startTime >= latestStart) {",
    run: JS(OG),
    expect: "대조군: 시작이 정확히 claimDeadline − minClaimWindow 이면 등록된다 (7차 M-7)",
  },
  {
    name: "7차 M-9: build-merkle 이 헤더 아래 # 줄을 다시 조용히 버린다",
    file: BM,
    from: "  if (commented.length) {",
    to:   "  if (false) {",
    run: JS(BG),
    expect: "헤더 아래 # 로 주석 처리된 수령자 행은 거부된다 (7차 M-9)",
  },
  {
    name: "7차 M-9: build-merkle 이 헤더 위의 # 줄까지 거부한다",
    file: BM,
    from: "    if (!t || t.startsWith('#')) continue;",
    to:   "    if (!t) continue;",
    run: JS(BG),
    expect: "대조군: 헤더 위의 # 줄과 빈 줄은 허용된다 (7차 M-9)",
  },
  {
    name: "7차 M-9: build-merkle 줄 번호가 실제 줄이 아니다",
    file: BM,
    from: "      line: i + 1,",
    to:   "      line: rows.length + 2,",
    run: JS(BG),
    expect: "그리고 오류 문구가 쓰는 줄 번호는 파일의 실제 줄 번호다 (7차 M-9)",
  },
  {
    name: "RoundSet: set-root 가 빌드에 없는 등록 라운드를 무시한다",
    file: SR,
    from: "  if (unknown.length) {",
    to:   "  if (false) {",
    run: JS(OG),
    expect: "빌드에 없는 번호(7 · 8)로 체인에 등록된 라운드가 있으면 거부된다 (14·15차 잔여)",
  },
  {
    name: "RoundSet: set-root 가 거절된 로그 범위를 줄이지 않는다",
    file: SR,
    from: "      if (range > 1) { range = Math.max(1, Math.floor(range / 2)); continue; }",
    to:   "      if (false) { continue; }",
    run: JS(OG),
    expect: "그리고 범위를 제한한 노드에서도 범위를 줄여 이벤트를 전부 읽었다 (머리 블록 포함)",
  },
  {
    name: "RoundSet: set-root 가 일시적 오류를 다시 시도하지 않는다",
    file: SR,
    from: "    for (let attempt = 0; attempt < 3 && logs === null; attempt++) {",
    to:   "    for (let attempt = 0; attempt < 1 && logs === null; attempt++) {",
    run: JS(OG),
    expect: "그리고 범위를 제한한 노드에서도 범위를 줄여 이벤트를 전부 읽었다 (머리 블록 포함)",
  },
  {
    name: "RoundSet: set-root 가 머리 블록을 빠뜨린다",
    file: SR,
    from: "  const headBlock = Number(await provider.send('eth_blockNumber', []));",
    to:   "  const headBlock = Number(await provider.send('eth_blockNumber', [])) - 1;",
    run: JS(OG),
    expect: "그리고 범위를 제한한 노드에서도 범위를 줄여 이벤트를 전부 읽었다 (머리 블록 포함)",
  },
  {
    name: "RoundSet: set-root 가 구간 사이 블록을 건너뛴다",
    file: SR,
    from: "    from = to + 1;\n  }\n  if (!sawConstructor) {",
    to:   "    from = to + 2;\n  }\n  if (!sawConstructor) {",
    run: JS(OG),
    expect: "LOG_RANGE=1 로 한 블록씩 읽어도 셋 다 찾는다 (구간 경계에서 빠지는 블록 없음)",
  },
  {
    name: "RoundSet: set-root 가 생성자 이벤트로 시작 블록을 증명하지 않는다",
    file: SR,
    from: "  if (!sawConstructor) {",
    to:   "  if (false) {",
    run: JS(OG),
    expect: "시작 블록이 claim 배포보다 늦으면 생성자 이벤트가 없어 멈춘다 (재재검 N1)",
  },
  {
    name: "RoundSet: set-root 가 아무 소유권 이전이나 생성자로 본다",
    file: SR,
    from: "        if (String(l.topics[1]).toLowerCase() === zeroTopic) sawConstructor = true;",
    to:   "        sawConstructor = true;",
    run: JS(OG),
    expect: "배포 바로 다음 블록부터여도 멈춘다: 레코드에 배포 블록이 없어도 시작 블록을 증명한다 (재재검 N1)",
  },
  {
    name: "RoundSet: set-root 가 모르는 라운드 탈출구를 무시한다",
    file: SR,
    from: "    if (!dryFlag('ALLOW_UNKNOWN_ROUNDS')) {",
    to:   "    if (true) {",
    run: JS(OG),
    expect: "ALLOW_UNKNOWN_ROUNDS=yes 면 경고하고 진행한다 (옛 빌드 파일을 잃은 경우의 탈출구)",
  },
  {
    name: "RoundSet: ALLOW_UNDERFUNDED 가 모르는 라운드 검사까지 끈다",
    file: SR,
    from: "    if (!dryFlag('ALLOW_UNKNOWN_ROUNDS')) {",
    to:   "    if (!dryFlag('ALLOW_UNKNOWN_ROUNDS') && !dryFlag('ALLOW_UNDERFUNDED')) {",
    run: JS(OG),
    expect: "ALLOW_UNDERFUNDED 로는 모르는 라운드 검사를 넘지 못한다 (자금 검사 플래그와 분리, 재검 F3)",
  },
  {
    name: "RoundSet: set-root 가 빌드에 있는 라운드까지 모르는 라운드로 센다",
    file: SR,
    from: "  const unknown = [...registeredIds].filter((id) => !fileIds.has(id) && id !== String(roundId));",
    to:   "  const unknown = [...registeredIds].filter((id) => id !== String(roundId));",
    run: JS(OG),
    expect: "대조군: 7번 · 8번의 파일을 빌드에 넣으면 등록되고 그 합계를 센다",
  },
  {
    name: "RoundSet: 스캔이 불완전해도 믿는다",
    file: SR,
    from: "  if (missed.length) {",
    to:   "  if (false) {",
    run: JS(OG),
    expect: "노드가 일부 로그를 빠뜨려 이 빌드의 등록된 라운드 이벤트가 없으면 불완전하다고 멈춘다 (재검 F2)",
  },
  {
    name: "RoundSet: 머리 블록보다 큰 시작 블록을 받는다",
    file: SR,
    from: "    if (fromBlock > headBlock) {",
    to:   "    if (false) {",
    run: JS(OG),
    expect: "머리 블록보다 큰 CLAIM_FROM_BLOCK 은 거부된다",
  },
  {
    name: "RoundSet: 기록된 배포 블록보다 늦은 시작 블록을 받는다",
    file: SR,
    from: "    if (recordedFrom !== null && fromBlock > recordedFrom) {",
    to:   "    if (false) {",
    run: JS(OG),
    expect: "기록된 배포 블록보다 늦은 CLAIM_FROM_BLOCK 은 거부된다 (재검 F2)",
  },
  {
    name: "RoundSet: 레코드의 배포 블록을 쓰지 않는다",
    file: SR,
    from: "  } else if (recordedFrom !== null) {",
    to:   "  } else if (false) {",
    run: JS(OG),
    expect: "대조군: 레코드의 claim.deployedBlock 부터 읽는다 (재검 F4)",
  },
  {
    name: "RoundSet: 메인넷에서도 0 번 블록부터 읽는다",
    file: SR,
    from: "  } else if (!mainnet) {",
    to:   "  } else if (true) {",
    run: JS(OG),
    expect: "메인넷에서 claim 배포 블록도 배포 트랜잭션도 기록돼 있지 않으면 시작 블록을 요구한다",
  },
  {
    name: "RoundSet: 기록된 배포 트랜잭션이 이 claim 을 만들었는지 보지 않는다",
    file: SR,
    from: "    if (String(rc.contractAddress || '').toLowerCase() !== claimAddr.toLowerCase()) {",
    to:   "    if (false) {",
    run: JS(OG),
    expect: "기록된 배포 트랜잭션이 다른 컨트랙트를 만들었으면 거부된다",
  },
  {
    name: "RoundSet: 영수증이 없을 때 원인을 말하지 않는다",
    file: SR,
    from: "    if (!rc) {",
    to:   "    if (false) {",
    run: JS(OG),
    expect: "영수증이 없으면 노드의 색인 삭제를 원인으로 안내한다 (재검 F4)",
  },
  {
    name: "RoundSet: deploy-claim 이 배포 블록을 기록하지 않는다",
    file: DCL,
    from: "      deployedBlock,",
    to:   "",
    run: JS(OG),
